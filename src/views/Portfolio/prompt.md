...
Fonts
Google Fonts import: Inter (300¨C700) and Instrument Serif (italic, 400).
- --font-body: "Inter", sans-serif ¡ú Tailwind font-body
- --font-display: "Instrument Serif", serif ¡ú Tailwind font-display
CSS Custom Properties (HSL, no hsl() wrapper ¡ª Tailwind adds it)
--bg: 0 0% 4%;
--surface: 0 0% 8%;
--text: 0 0% 96%;
--muted: 0 0% 53%;
...
Accent Gradient
linear-gradient(90deg, #89AACC 0%, #4E85BF 100%) ¡ª used on logo ring, hover borders, progress bars. CSS utility class .accent-gradient.
Custom Animations (in index.css)
- @keyframes scroll-down ¡ª translateY(-100%) ¡ú translateY(200%), 1.5s ease-in-out infinite
- @keyframes role-fade-in ¡ª opacity 0 + translateY(8px) ¡ú opacity 1 + translateY(0), 0.4s ease-out
- @keyframes gradient-shift ¡ª background-position 0% 50% ¡ú 100% 50% ¡ú 0% 50%, 6s ease infinite (for animated gradient borders)
Forced dark theme ¡ª no light mode toggle. body gets bg-bg text-text-primary.
---
Page Structure (Index.tsx)
{isLoading && <LoadingScreen onComplete={() => setIsLoading(false)} />}
---
Section 1: Loading Screen
Full-screen overlay (fixed inset-0 z-[9999] bg-bg). Uses requestAnimationFrame counter from 000¡ú100 over 2700ms.
- Top-left: "Portfolio" label ¡ª text-xs text-muted uppercase tracking-[0.3em]. Animates y:-20¡ú0, opacity 0¡ú1.
- Center: Rotating words ["Design", "Create", "Inspire"] cycling every 900ms. AnimatePresence mode="wait" with y:20¡ú0¡ú-20 transitions. text-4xl md:text-6xl lg:text-7xl font-display italic text-text-primary/80.
- Bottom-right: Counter display ¡ª text-6xl md:text-8xl lg:text-9xl font-display text-text-primary tabular-nums. Shows String(count).padStart(3, "0").
...
- Uses hls.js ¡ª if Hls.isSupported(), create HLS instance; else if native HLS support, set video.src directly.
...
1. Logo: 9¡Á9 circle with accent gradient border (reverses direction on hover). Inner bg-bg circle with "JA" in font-display italic text-[13px]. Scales 110% on hover.
2. Divider: w-px h-5 bg-stroke mx-1 (hidden on mobile)
3. Nav links: ["Home", "Work", "Resume"] ¡ª text-xs sm:text-sm rounded-full px-3 sm:px-4 py-1.5 sm:py-2. Active: text-text-primary bg-stroke/50. Inactive: text-muted hover:text-text-primary hover:bg-stroke/50.
4. Divider
5. "Say hi" button: Same size as nav links. On hover, shows accent gradient border behind (using absolute span with inset: -2px). Inner content wrapped in bg-surface rounded-full backdrop-blur-md. Includes "¡ú" arrow.
Hero Content (centered, z-10)
- Eyebrow: text-xs text-muted uppercase tracking-[0.3em] mb-8 ¡ª "COLLECTION '26". Class blur-in.
- Name: text-6xl md:text-8xl lg:text-9xl font-display italic leading-[0.9] tracking-tight text-text-primary mb-6 ¡ª "Michael Smith". Class name-reveal.
- Role line: "A {role} lives in Chicago." ¡ª roles cycle every 2s through ["Creative", "Fullstack", "Founder", "Scholar"]. Role word uses font-display italic text-text-primary animate-role-fade-in inline-block with key={roleIndex} for re-triggering animation.
- Description: text-sm md:text-base text-muted max-w-md mb-12 ¡ª "Designing seamless digital interactions by focusing on the unique nuances which bring systems to life."
...
- .name-reveal: opacity 0¡ú1, y 50¡ú0, duration 1.2s, delay 0.1s
- .blur-in: opacity 0¡ú1, filter blur(10px)¡úblur(0px), y 20¡ú0, duration 1s, stagger 0.1, delay 0.3s
Scroll Indicator
...
Header
Framer Motion whileInView ¡ª opacity 0¡ú1, y 30¡ú0, duration 1s, ease [0.25,0.1,0.25,1], viewport once margin "-100px".
- Eyebrow: w-8 h-px bg-stroke + "Selected Work" text-xs text-muted uppercase tracking-[0.3em]
- Heading: "Featured projects" ¡ª italic word in font-display italic
- Subtext: "A selection of projects I've worked on, from concept to launch."
- "View all work" button (desktop only, hidden md:inline-flex) ¡ª rounded-full with gradient hover border ring + right arrow
Bento Grid
...
- Halftone overlay: radial-gradient(circle, #000 1px, transparent 1px) at 4¡Á4px, opacity-20 mix-blend-multiply
- Hover: bg-bg/70 opacity-0¡ú1 + backdrop-blur-lg
- Hover label: pill with animated gradient border, white bg, "View ¡ú Title" (title in font-display italic)
---
Section 4: Journal
...
GSAP Marquee
"BUILDING THE FUTURE ¡ª" repeated 10¡Á. GSAP xPercent: -50, duration 40, ease "none", repeat -1.
CTA
